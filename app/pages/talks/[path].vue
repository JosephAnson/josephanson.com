...
if (talk.value && talk.value.seo.image) {
  useHead({
    meta: [
      { property: 'og:image', content: talk.value.seo.image },
    ],
  })
}
...
<template>
  <BaseContainer class="relative z-10 text-sm md:text-base">
    <article ref="article" class="slide-enter-content relative pb-30 md:pb-100">
      <NuxtLink
        :to="parentPath"
      ...
      <div class="grid-cols-2 w-full gap-12 md:grid">
        <div class="mt-4 w-full">
          <iframe :src="talk.url" class="aspect-video w-full border border-gray-200" />
